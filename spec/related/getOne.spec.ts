...
describe('Related#getOne simple', () => {
  describe('without alias', () => {
    describe('from definition', () => {
      ...
      it('returns a random defined element', () => {
        const subject: IElementDefinition = service.getOne([{type: 'item1'}]);
        expect(subject.type).to.eql('item1');
        expect(subject.text).to.be.oneOf(['item_1_1', 'item_1_2']);
      });

      it('returns an existing defined element', () => {
        const subject: IElement = service.getOne([{type: 'item1', random: ERandomOption.exists}]);
        expect(subject.type).to.eql('item1');
    ...
    describe('from existing', () => {
      ...
      it('returns a random defined element', () => {
        const subject: IElement = service.getOne([{type: 'item1', random: ERandomOption.all}]);
        expect(subject.type).to.eql('item1');
